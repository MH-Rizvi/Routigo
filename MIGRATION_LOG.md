...
This document details the complete migration of the routigo backend infrastructure from a local, single-node SQLite stack to a cloud-native Supabase PostgreSQL and Supabase Auth implementation. This is designated as **Phase 20: Supabase Migration**.

---

## 1. What We Had Before (Original Stack)
Initially, routigo was built for rapid prototyping and local development validation:
- **Relational Database**: A local `routigo.db` file (SQLite) managed via SQLAlchemy.
...
## 6. Database Schema
Executed manually via the Supabase Admin SQL Panel, these structured queries initialized the core routigo schema on Postgres:
